...
let state,turn,selected=null,moves=[],history=[],castling,enPassant=null,gameOver=false,lastMove=null,positionHistory=[],halfmoveClock=0;

function clone(){return {state:state.map(r=>r.slice()),turn,castling:{...castling},enPassant:enPassant&&[...enPassant],lastMove:lastMove&&JSON.parse(JSON.stringify(lastMove)),positionHistory:[...positionHistory],halfmoveClock}}
function fresh(){state=[
['r','n','b','q','k','b','n','r'],['p','p','p','p','p','p','p','p'],
['','','','','','','',''],['','','','','','','',''],['','','','','','','',''],['','','','','','','',''],
['P','P','P','P','P','P','P','P'],['R','N','B','Q','K','B','N','R']];
turn='w';selected=null;moves=[];history=[];castling={wK:true,wQ:true,bK:true,bQ:true};enPassant=null;gameOver=false;lastMove=null;halfmoveClock=0;positionHistory=[positionKey()];render()}
...
function positionKey(){
 ...
 return state.map(r=>r.map(p=>p||'.').join('')).join('/')+' '+turn+' '+rights+' '+ep;
}
function repetitionCount(){const k=positionKey();return positionHistory.filter(x=>x===k).length}
...
function makeMove(fr,fc,tr,tc,simulate=false){
 const piece=state[fr][fc],side=color(piece),target=state[tr][tc];const isPawn=piece.toLowerCase()==='p';const isCapture=!!target||(isPawn&&enPassant&&tr===enPassant[0]&&tc===enPassant[1]);state[tr][tc]=piece;state[fr][fc]='';
 ...
 if(!simulate){lastMove={fr,fc,tr,tc};halfmoveClock=(isPawn||isCapture)?0:halfmoveClock+1;}
}
...
function finishStatus(){const check=isInCheck(turn),available=allLegal(turn);if(!available.length){gameOver=true;statusEl.textContent=check?(turn==='w'?'Checkmate — Black wins!':'Checkmate — White wins!'):'Draw — Stalemate';return}if(halfmoveClock>=100){gameOver=true;statusEl.textContent='Draw — Fifty-move rule';return}if(repetitionCount()>=3){gameOver=true;statusEl.textContent='Draw — Threefold repetition';return}const pieces=[];for(let R=0;R<8;R++)for(let C=0;C<8;C++)if(state[R][C]&&state[R][C].toLowerCase()!=='k')pieces.push(state[R][C].toLowerCase());if(!pieces.length||pieces.every(p=>p==='b'||p==='n')){gameOver=true;statusEl.textContent='Draw — Insufficient material';return}statusEl.textContent=check?(turn==='w'?'White is in CHECK — you must escape the check!':'Black is in CHECK — you must escape the check!'):(turn==='w'?'White to move':'Black to move')}
...
function click(r,c){if(gameOver)return;const p=state[r][c];if(selected&&moves.some(m=>m[0]===r&&m[1]===c)){history.push(clone());makeMove(selected[0],selected[1],r,c);selected=null;moves=[];turn=enemyOf(turn);positionHistory.push(positionKey());render();return}if(p&&color(p)===turn){selected=[r,c];moves=legalMoves(r,c)}else{selected=null;moves=[]}render()}
document.getElementById('restart').onclick=fresh;document.getElementById('undo').onclick=()=>{const h=history.pop();if(h){state=h.state;turn=h.turn;castling=h.castling;enPassant=h.enPassant;lastMove=h.lastMove;positionHistory=h.positionHistory||[positionKey()];halfmoveClock=h.halfmoveClock||0;selected=null;moves=[];gameOver=false;render()}};fresh();